...
plt.bar(x, y, width=1, align='center', color='green', alpha=0.6)
plt.axvline(x=failure_turn, linewidth=1.5, color='black')
plt.annotate('Failure turn', xy=(failure_turn - 0.7,  (max(y) + max(y)*0.7)/2), rotation='vertical', size='11', verticalalignment='center')
# plt.axvline(x=failure_turn + 3, linewidth=1.5, color='black')

for i, j in zip(x,y):
    plt.annotate('{} \%'.format(round(j,4)), xy=(i, j+ 0.03*j), rotation='vertical', size='7', verticalalignment='bottom', horizontalalignment='center')
    if i == 8:
        t_9 = j
        plt.annotate('After 3 turns: {} \%'.format(round(t_9, 3)), xy=(0.5, max(y)+0.4*max(y)), size='7')
    else:
        continue
    if i == 15:
        t_15 = j
        plt.annotate('After 10 turns: {} \%'.format(round(t_15, 3)), xy=(0.5, max(y)+0.3*max(y)), size='7')
    else:
        continue
