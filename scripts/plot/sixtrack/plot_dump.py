...
phi, delta, h = get_bucket('SPS_inj')  # Longitudinal contour
...
sigma_z_x = get_sigma_ellipse(0.0151, 7.16610376198e-06, 0, -7.5e-4, 10)
sigma_z_y = get_sigma_ellipse(0.0151, 7.12405639107e-06, 0, 0, 10)
...
for turn in turn_range:
    ...
    ax5.set_xlim([-5e-2, 5e-2])
    ax5.set_ylim([-12e-3, 12e-3])

    ax6 = fig.add_subplot(326)
    plot_2d_hist(z_plot, e_plot, nbins)
    plt.contour(2*phi, delta, 2*h, 40, linewidths=0.2, cmap='terrain_r')
    plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))
    ax6.set_ylabel(r"$\Delta E / E$")
    ax6.set_xlabel(r"$z$ [m]")
    # ax6.set_xlim([-0.8, 0.8])
    # ax6.set_ylim([-8e-4, 8e-4])
    ax6.set_xlim([-0.8, 0.8])
    ax6.set_ylim([-8e-3, 8e-3])
